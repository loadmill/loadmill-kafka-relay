...
export class SubscribersManager {
  ...
  getActiveSubscribers(): Subscribers {
    return this.subscribers;
  }

  async isSubscriberExists(id: string): Promise<boolean> {
    return !!this.subscribers[id];
  }

  async getMessages(subscriberId: string): Promise<ConsumedMessage[]> {
  ...
  protected async deleteExpiredSubscribers(): Promise<void> {
